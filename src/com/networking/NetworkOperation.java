...
import com.networking.external.Base64;
import com.networking.external.CachingInputStream;
import com.networking.external.MD5;
...
public class NetworkOperation implements Runnable {
    ...
    private int status;
    private boolean useJSONStringForPost = false;
    ...
    private int prepareRequest() {
        ...
        if (httpMethod == HttpMethod.POST || httpMethod == HttpMethod.PUT) {
            List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>();
            if (useJSONStringForPost) {
            	try {
            		StringEntity se = null;
            		for (String param : params.keySet()) {
            			 se = new StringEntity(params.get(param));
            			 break;
                    }
    ...
    public void setJSONStringForPost(boolean yesNo) {
    	useJSONStringForPost = yesNo;
    }
